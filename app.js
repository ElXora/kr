...
if (cluster.isMaster) {
  ...
  cluster.on('exit', (worker, code, signal) => {
    ...
    crashResetTimer = setTimeout(() => { crashCount = 0; }, 10000);
    if (crashCount > 5) {
      console.log(chalk.red(`Workers crashing too fast (${crashCount}x). Stopping auto-restart.`));
      console.log(chalk.red(`Fix your settings.json then run: pm2 restart kroxy`));
      return;
    }
    console.log(chalk.red(`Worker ${worker.process.pid} died. Restarting in 2s...`));
