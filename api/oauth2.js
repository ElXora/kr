...
module.exports.load = async function (app, db) {
  ...
  app.get(settings.api.client.oauth2.callbackpath, async (req, res) => {
    ...
    // Fix: redirect directly to /submitlogin instead of showing a spinner
    // with history.pushState. The spinner page changed the URL to /login
    // before the redirect fired — if anything was slow, the browser retried
    // with the already-consumed Discord code and got stuck on "Logging in...".
    const safeCode = encodeURIComponent(req.query.code.replace(/'/g, ""));
    return res.redirect(`/submitlogin?code=${safeCode}`);
  });
